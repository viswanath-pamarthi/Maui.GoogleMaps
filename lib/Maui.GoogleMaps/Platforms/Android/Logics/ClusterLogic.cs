...
using Maui.GoogleMaps.Platforms.Clustering.Android;
...
namespace Maui.GoogleMaps.Platforms.Logics.Android
{
    ...
    {
        protected override IList<Pin> GetItems(Map map) => base.Map.Pins;
        ...
        internal override void Register(GoogleMap oldNativeMap, Map oldMap, GoogleMap newNativeMap, Map newMap, IElementHandler handler)
        {
            ...
            clusterManager = new ClusterManager(context, newNativeMap) { Algorithm = GetClusterAlgorithm() };
            clusterHandler = new ClusterLogicHandler(newMap, clusterManager, this);
            newMap.OnCluster = HandleClusterRequest;
            newNativeMap.CameraIdle += NativeMapOnCameraIdle;
            newNativeMap.SetOnMarkerClickListener(clusterManager);
            newNativeMap.SetOnInfoWindowClickListener(clusterManager);
        ...
        private IAlgorithm GetClusterAlgorithm()
        {
            ...
            switch (Map.ClusterOptions.Algorithm)
            {
        ...
        {
            var cameraPosition = base.NativeMap.CameraPosition;
            if (previousCameraPostion == null || Math.Abs(previousCameraPostion.Zoom - cameraPosition.Zoom) > 0.001)
            {
                previousCameraPostion = base.NativeMap.CameraPosition;
        ...
        internal override void Unregister(GoogleMap nativeMap, Map map)
        {
            if (nativeMap != null)
            {
                base.NativeMap.CameraIdle -= NativeMapOnCameraIdle;
                base.NativeMap.SetOnMarkerClickListener(null);
                base.NativeMap.SetOnInfoWindowClickListener(null);
        ...
        protected override ClusteredMarker DeleteNativeItem(Pin outerItem)
        {
            ...
            if (ReferenceEquals(base.Map.SelectedPin, outerItem))
                base.Map.SelectedPin = null;
        ...
        internal override void OnMapPropertyChanged(string propertyName)
        {
            ...
            {
                if (!onMarkerEvent)
                    UpdateSelectedPin(base.Map.SelectedPin);
                base.Map.SendSelectedPinChanged(base.Map.SelectedPin);
            }
        }
        ...
        private void UpdateSelectedPin(Pin pin)
        {
            if (pin == null)
            {
                foreach (var outerItem in GetItems(base.Map))
                {
        ...
        private async void TransformXamarinViewToAndroidBitmap(Pin outerItem, ClusteredMarker nativeItem)
        {
            if (outerItem?.Icon?.Type == BitmapDescriptorType.View && outerItem.Icon?.View != null)
            {
                ...
                var nativeView = await Utils.ConvertMauiToNative(
                    iconView, Handler);
        ...
        protected override void OnUpdateIsVisible(Pin outerItem, ClusteredMarker nativeItem)
        {
            ...
            if (!isVisible && ReferenceEquals(base.Map.SelectedPin, outerItem))
            {
                base.Map.SelectedPin = null;
            }
        }
        ...
        private void OnMarkerDragStart(object sender, GoogleMap.MarkerDragStartEventArgs e)
        {
            ...
            base.Map.SendPinDragStart(draggingPin);
        }
        ...
        private void OnMarkerDrag(object sender, GoogleMap.MarkerDragEventArgs e)
        {
            if (draggingPin == null) return;
            UpdatePositionWithoutMove(draggingPin, e.Marker.Position.ToPosition());
            base.Map.SendPinDragging(draggingPin);
        }
        ...
        private void OnMarkerDragEnd(object sender, GoogleMap.MarkerDragEndEventArgs e)
        {
            if (draggingPin != null)
            {
                UpdatePositionWithoutMove(draggingPin, e.Marker.Position.ToPosition());
                RefreshClusterItem();
                base.Map.SendPinDragEnd(draggingPin);
                draggingPin = null;
            }
            withoutUpdateNative = false;
        }
